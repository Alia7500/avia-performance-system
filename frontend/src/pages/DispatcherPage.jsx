...
const DispatcherPage = ({ user, onLogout }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-500 overflow-hidden">
      ...
      {/* --- SIDEBAR --- */}
      {/* print:hidden скрывает меню при печати PDF */}
      <aside className="w-72 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 flex flex-col shadow-2xl z-30 print:hidden">
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="sticky top-0 z-20 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 p-6 px-10 flex justify-between items-center print:hidden">
          <div>
        ...
        <div className="p-10 max-w-[1600px] mx-auto print:p-0">
                        ...
                        <div className="flex justify-between items-start mb-6">
                          <div>
                            <div className="flex items-center gap-3 mb-3">
                              <span className="px-3 py-1.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-[10px] font-black uppercase tracking-widest border border-slate-300 dark:border-slate-600">
                                БОРТ: {f.tail?.replace('RA-', '') || 'ОЖИДАНИЕ'}
                              </span>
                              {f.status === 'Задержан' && (
             ...
             <div className="bg-white dark:bg-slate-800 p-8 rounded-[3rem] shadow-2xl animate-in fade-in duration-500 border border-slate-200 dark:border-slate-700">
                ...
                <div className="w-full h-[700px] rounded-[2.5rem] overflow-hidden border-8 border-slate-50 dark:border-slate-900 relative z-0">
                  <MapContainer center={[55.75, 37.61]} zoom={4} style={{ height: '100%', width: '100%' }} attributionControl={false}>
                    <TileLayer url={isDarkMode ? 'https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}&hl=ru&apistyle=s.t%3A33|s.e%3Ag|p.c%3A%23242f3e' : 'https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}&hl=ru'} />
                    {monitorData.map((f, i) => {
                      ...
                      const heading = Math.atan2(arrCoords[1] - depCoords[1], arrCoords[0] - depCoords[0]) * (180 / Math.PI) + 90;
                      const hasRisk = (f.crew || []).some(c => c.score > 0 && c.score < 70);

                      return (
                        <React.Fragment key={i}>
                          <Marker position={depCoords} icon={L.divIcon({className: 'bg-blue-600 w-3 h-3 rounded-full border-2 border-white shadow-lg'})}><Popup><b className="font-sans text-sm">{f.dep}</b></Popup></Marker>
                          <Marker position={arrCoords} icon={L.divIcon({className: 'bg-slate-400 w-3 h-3 rounded-full border-2 border-white shadow-lg'})}><Popup><b className="font-sans text-sm">{f.arr}</b></Popup></Marker>
          ...
          {activeTab === 'report' && (
            <div className="max-w-4xl mx-auto bg-white text-slate-900 p-12 rounded-[2rem] shadow-2xl print:shadow-none print:p-0 print:w-full">
              
              <div className="flex justify-end mb-8 print:hidden">
                 <button onClick={() => window.print()} className="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-colors flex items-center gap-2">
                    <Download size={18}/> Экспорт в PDF
                 </button>
              </div>
              ...
              <div className="border-b-4 border-slate-900 pb-6 mb-8 flex justify-between items-end">
                 ...
                 <div className="text-right">
                    <p className="text-sm font-bold">Дата: {new Date().toLocaleDateString()}</p>
              ...
              <div className="bg-slate-100 p-6 rounded-2xl mb-8 border border-slate-300">
                 ...
                 <p className="text-sm font-medium leading-relaxed">
                    На момент формирования отчета в воздухе находится <b>{monitorData.length}</b> бортов типа МС-21-300. 
                    Общая численность экипажа на смене: <b>{monitorData.reduce((a, f) => a + (f.crew?.length || 0), 0)}</b> чел. 
                    {riskFlightsCount === 0 
                      ? " Аномалий биометрии не выявлено. Состояние летного состава штатное. Экипажи готовы к выполнению полетных заданий." 
                      : ` ВНИМАНИЕ: Зафиксировано ${riskFlightsCount} борта(ов), экипажи которых находятся в зоне физиологического риска (ЧСС выше нормы / высокий стресс). Рекомендуется усилить медицинский контроль.`}
                 </p>
              </div>
              ...
              <table className="w-full text-left text-sm border-collapse">
                 ...
                 <tbody>
                   {monitorData.map((f, i) => {
                     const safeCrew = f.crew || [];
                     const avgScore = safeCrew.length > 0 ? Math.round(safeCrew.reduce((s, c) => s + c.score, 0) / safeCrew.length) : 0;
                     return (
                       <tr key={i} className="border-b border-slate-300">
                         <td className="p-3 border-x border-slate-300 font-bold">{f.tail?.replace('RA-', '')} ({f.number})</td>
                         <td className="p-3 border-x border-slate-300">{f.dep} ➔ {f.arr}</td>
                         <td className="p-3 border-x border-slate-300 text-center font-mono font-bold">{avgScore}%</td>
                         <td className="p-3 border-x border-slate-300 text-center font-bold">
                            {avgScore > 70 ? <span className="text-emerald-600">ШТАТНО</span> : <span className="text-rose-600">КОНТРОЛЬ</span>}
                         </td>
                       </tr>
                     );
                   })}
