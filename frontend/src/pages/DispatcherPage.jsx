...
import { 
  ...
  Activity, BrainCircuit, Navigation, Clock, Info
} from 'lucide-react';
...
const createPlaneIcon = (heading, isRisk) => {
  const color = isRisk ? '#f43f5e' : '#3b82f6'; // Красный или Синий
                ...
                {monitorData.map((f, i) => {
                  ...
                  return (
                    <div key={i} className={`bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-xl border-2 transition-all duration-500 overflow-hidden ${alerts > 0 ? 'border-rose-500 shadow-rose-500/20' : 'border-slate-200 dark:border-slate-700'}`}>
                      ...
                      {/* СПИСОК ЭКИПАЖА И ГРАФИКИ (РАСКРЫВАЕТСЯ) */}
                      {isExpanded && (
                                       ...
                                       <AreaChart data={sel.history ||[]}>
                                         <defs>
                                           <linearGradient id="colorHrPersonal" x1="0" y1="0" x2="0" y2="1">
                                             <stop offset="5%" stopColor={sel.score > 70 ? "#10b981" : "#f43f5e"} stopOpacity={0.4}/>
                                             <stop offset="95%" stopColor={sel.score > 70 ? "#10b981" : "#f43f5e"} stopOpacity={0}/>
                                           </linearGradient>
                                         </defs>
             ...
             <div className="bg-white dark:bg-slate-800 p-8 rounded-[3rem] shadow-2xl animate-in fade-in duration-500 border border-slate-200 dark:border-slate-700">
                ...
                <div className="w-full h-[700px] rounded-[2.5rem] overflow-hidden border-8 border-slate-50 dark:border-slate-900 relative z-0">
                  <MapContainer center={[55.75, 37.61]} zoom={4} style={{ height: '100%', width: '100%' }} attributionControl={false}>
                    {/* РУССКАЯ КАРТА Google (БЕЗ ФЛАГОВ И КОПИРАЙТОВ) */}
                    <TileLayer 
                    ...
                    {monitorData.map((f, i) => {
                      ...
                      return (
                        <React.Fragment key={i}>
                          ...
                          {depCoords && arrCoords && (
                            <>
                              <Polyline positions={[depCoords, arrCoords]} color="#94a3b8" weight={2} opacity={0.5} dashArray="5, 8" />
                              {f.lat && f.lon ? (
                                // Если есть реальные координаты с радара
                                <Marker position={[f.lat, f.lon]} icon={createPlaneIcon(f.heading, hasRisk)}>
                                  <Popup>
                                    <div className="text-center font-sans p-2"><b className="text-blue-600 text-xl block mb-1">{f.number}</b><span className="font-black text-slate-500 uppercase text-[10px] bg-slate-100 px-2 py-1 rounded">{f.dep} ➔ {f.arr}</span></div>
                                  </Popup>
                                </Marker>
                              ) : (
                                // Если нет координат, рисуем самолет по расчетному прогрессу
                                (() => {
                                  const prog = calcFlightProgress(f.time_dep, f.time_arr, f.delay) / 100;
                                  if (prog > 0 && prog < 1) {
                                    const clat = depCoords[0] + (arrCoords[0] - depCoords[0]) * prog;
                                    const clon = depCoords[1] + (arrCoords[1] - depCoords[1]) * prog;
                                    const hdg = Math.atan2(arrCoords[1] - depCoords[1], arrCoords[0] - depCoords[0]) * (180 / Math.PI) + 90;
                                    return (
                                      <Marker position={[clat, clon]} icon={createPlaneIcon(hdg, hasRisk)}>
                                        <Popup>
                                          <div className="text-center font-sans p-2"><b className="text-blue-600 text-xl block mb-1">{f.number}</b><span className="font-black text-slate-500 uppercase text-[10px] bg-slate-100 px-2 py-1 rounded">{f.dep} ➔ {f.arr}</span></div>
                                        </Popup>
                                      </Marker>
